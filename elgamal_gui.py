...
def on_manual_entry():
    try:
        ...
        if block_size.get() == 2:
        	prime_minlimit = 256 ** 2 
        	if not (p >= prime_minlimit):
            		result_message.set("Error: p must be equal or greater than 65536 for Double Blocks.")
            		return
            		
        if block_size.get() == 1:
        	prime_minlimit = 256 ** 2 
        	if not (p >= prime_minlimit):
            		result_message.set("Error: p must be equal or greater than 256 for Simple Blocks.")
            		return
